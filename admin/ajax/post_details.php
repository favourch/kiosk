<?php 
	...
	if($type_of_post=="event"){
		 ...
		 $row_post_events=mysql_fetch_assoc($select_post_events);
		 		$event_id=$row_post_events['event_id'];
				...
				$query_img=mysql_query("SELECT * FROM attachment_t WHERE post_id='$event_id'");
				$imgs_value="";
				while($row_img=mysql_fetch_assoc($query_img)){
					$img_name=$row_img['attachment_name'];

					$imgs_value.="<a href='../e-bulletin/images/imgs_post/".$img_name."' class='ui-lightbox'><img src='../e-bulletin/images/imgs_post/".$img_name."' width='111px' height= '114px'></a>";
				}
		...
		$query_img=mysql_query("SELECT * FROM attachment_t WHERE post_id='$message_id'");
				$imgs_value="";
				while($row_img=mysql_fetch_assoc($query_img)){
					$img_name=$row_img['attachment_name'];
		 ...
		 $return_array = array( 
			...
			$row_post=mysql_fetch_assoc($select_post);
					$holiday_id=$row_post['holiday_id'];
					$holidate=date("F j, Y", strtotime($row_post['date_of_holiday']));
			
			$query_img=mysql_query("SELECT * FROM attachment_t WHERE post_id='$holiday_id'");
				$imgs_value="";
				while($row_img=mysql_fetch_assoc($query_img)){
					$img_name=$row_img['attachment_name'];
