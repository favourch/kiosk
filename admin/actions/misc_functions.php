...
function getName(){
	...
	if(isset($_SESSION['kiosk']['user_id'])){
	 ...
	 // echo $user_id;
	$query_acc=mysql_query("SELECT * FROM account_t WHERE account_no='$user_id'") or die(mysql_error());
	$row_acc=mysql_fetch_assoc($query_acc);
	$type=$row_acc['type'];

		if($type=='admin'){
			$name=$row_acc['username'];
			return $name;
		}
		else if ($type=='personnel'){
			$query_name=mysql_query("SELECT * FROM personnel_members_t, personnel_t WHERE personnel_members_t.account_no='$user_id' 
			...
			$query_name=mysql_query("SELECT * FROM student_members_t, student_t WHERE student_members_t.account_no='$user_id' 
									AND student_members_t.student_id=student_t.student_id");
			if(mysql_num_rows($query_name)>0){
				
				$row_name=mysql_fetch_assoc($query_name);
				$student_id=$row_name['student_id'];
			...
			$fname=$row_name['f_name'];
			//echo $fname;
			$lname=$row_name['l_name'];
			$name=$fname.' '.$lname;
...
function getAccountName($account_no){
	...
	function privileges($account_no){

    		$query_account = mysql_query("SELECT * FROM account_t WHERE account_no='{$account_no}'") or die(trigger_error(mysql_error()));
    		$row_account = mysql_fetch_assoc($query_account);
